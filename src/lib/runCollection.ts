...
export interface CollectionResult {
  status: RunStatus;
  admob: { ok: boolean; rows?: number; error?: string };
}

// Full data collection — identical to the daily cron's work. Shared by the
// scheduled `/api/cron` route and the on-demand `/api/refresh` button so both
// stay in lockstep. Must finish within the 60s Vercel Hobby function cap; all
// per-app collectors already run in parallel inside `runDailyCollection`.
export async function runFullCollection(): Promise<CollectionResult> {
  ...
  // their own append-only files. Never fails the collection.
  let admob: CollectionResult["admob"] = { ok: false };
